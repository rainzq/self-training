...
void empty(ErrorContext &ec)
{
    ...
    // If this test fails, it means that get_rows returned a non-zero value
    // when called on an "empty" (0x0) matrix created with the default
    // constructor.
    ec.DESC("default constructor and get_rows");
    
    // Construct an empty matrix using the default constructor.
    const Matrix a;
    
    // Make sure 'get_rows' method returns zero for this matrix.
    ec.result(a.get_rows() == 0);
    
    
    // Same as above, for get_cols instead of get_rows.
    ec.DESC("default constructor and get_cols");
    ec.result(a.get_cols() == 0);
    
    
    // If this test fails, it means that get_rows returned a non-zero value
    // when called on an "empty" (0x0) matrix created with the 2-argument
    // constructor.
    ec.DESC("two-argument constructor and get_rows");
    
    // Construct an empty matrix using the two-argument constructor.
    const Matrix b(0, 0);
    
    // Make sure 'get_rows' method returns zero for this matrix.
    ec.result(b.get_rows() == 0);
    
    // Same as above, for get_cols instead of get_rows.
    ec.DESC("two-argument constructor and get_cols");
    ec.result(b.get_cols() == 0);
}
...
void basic(ErrorContext &ec, int level)
{
    ...
    
    {
        ...
        // Make sure get_rows and get_cols work for non-empty matrices.
        ec.DESC("get_rows and get_cols");
        Matrix a(rows, cols);
        ec.result((a.get_rows() == rows) && (a.get_cols() == cols));
    }
    ...
    for (int pass = 1; pass <= level; pass++)
    {
        ...
        ostringstream oss;
        oss << "get_elem and set_elem (1), pass " << pass << ": element (" << r
            << "," << c << ")" << ends;
        ec.DESC(oss.str());
    ...
    {
        {
            ...
            ostringstream oss;
            oss << "get_elem and set_elem (2a), pass " << pass 
                << " [read]" << ends;
            ec.DESC(oss.str());
        }
        ...
        for (int r = 0; r < rows; r++)
        {
            for (int c = 0; c < cols; c++)
            {
                good &= (a.get_elem(r, c) == ((r - 666 * c) * pass));
            }
        }
...
void copy(ErrorContext &ec)
{
    ...
    ec.result((b.get_rows() == 123) && (b.get_cols() == 456));
    ...
    good = (a.get_rows() == 123) && (a.get_cols() == 456);
    
    for (int r = 0; r < 123; r++)
    {
        for (int c = 0; c < 123; c++)
        {
            good &= (a.get_elem(r, c) == r * (c - 246));
        }
    }
    ...
    good = (a.get_rows() == 23) && (a.get_cols() == 5);
...
void comp(ErrorContext &ec, int level)
{
    ...
    for (int pass = 1; pass <= (level / 3) * (level / 3); pass++)
    {
        ...
        a.set_elem(row, col, a.get_elem(row, col) + 1);
    ...
    for (int pass = 1; pass <= level / 2; pass++)
    {
        ...
        for (int ix = 0; ix < x; ix++)
        {
            for (int iy = 0; iy < y; iy++)
            {
                ...
                a_plus_b.set_elem(ix, iy, va + vb);
                a_minus_b.set_elem(ix, iy, va - vb);
            }
        }
        ...
        for (int iy = 0; iy < y; iy++)
        {
            for (int iz = 0; iz < z; iz++)
            {
                ...
                for (int ix = 0; ix < x; ix++)
                {
                    a_times_c.set_elem(ix, iz, a_times_c.get_elem(ix, iz) 
                                      + vc * a.get_elem(ix, iy));
                }
            }
        }
