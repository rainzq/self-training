...
static struct command conf_commands[] = {
    ...
    { string("zookeeper"),
      conf_set_zookeeper,
      offsetof(struct conf_pool, zookeeper) },
...
conf_pool_init(struct conf_pool *cp, struct string *name)
{
    ...
    string_init(&cp->zookeeper.pname);
    string_init(&cp->zookeeper.node);
...
conf_pool_deinit(struct conf_pool *cp)
{
    ...
    string_deinit(&cp->zookeeper.pname);
    string_deinit(&cp->zookeeper.node);
...
conf_pool_each_transform(void *elem, void *data)
{
    ...
    sp->name = cp->name;
    sp->zk_addrstr = cp->zookeeper.pname;
    sp->zk_node = cp->zookeeper.node;
...
conf_dump(struct conf *cf)
{
    ...
    for (i = 0; i < npool; i++) {
        ...
        log_debug(LOG_VVERB, "  zookeeper: %.*s",
                  cp->zookeeper.pname.len, cp->zookeeper.pname.data);
        log_debug(LOG_VVERB, "  zookeeper node: %.*s",
                  cp->zookeeper.node.len, cp->zookeeper.node.data);
...
char *
conf_set_zookeeper(struct conf *cf, struct command *cmd, void *conf)
{
    rstatus_t status;
    struct string *value;
    struct conf_zookeeper *field;
    uint8_t *p;

    p = conf;
    field = (struct conf_zookeeper *)(p + cmd->offset);
...
char *
conf_set_zk_node(struct conf *cf, struct command *cmd, void *conf)
{
    rstatus_t status;
    struct string *value;
    struct conf_zookeeper *field;
    ...
    status = string_duplicate(&field->node, value);
    if (status != NC_OK) {
        return CONF_ERROR;
    }
