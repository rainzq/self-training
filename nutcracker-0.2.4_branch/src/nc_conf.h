...
struct conf_zookeeper {
    struct string   pname;   /* server: as "ip:port,ip:port,ip:port" */
...
struct conf_pool {
    ...
    struct conf_zookeeper zookeeper;          /* zookeeper: */
...
char *conf_set_zookeeper(struct conf *cf, struct command *cmd, void *conf);
char *conf_set_zk_node(struct conf *cf, struct command *cmd, void *conf);
